...
for color in colors:
    ...
    # create color folder
    if not os.path.exists(os.path.join(OUTPUT,color)):
        os.mkdir(os.path.join(OUTPUT,color))
    ...
    for idx,img in enumerate(imgs):
        ...
        cv2.imwrite(os.path.join(OUTPUT,color,img),cv2.cvtColor(box,cv2.COLOR_RGB2BGR))
